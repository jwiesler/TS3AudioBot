...
namespace TS3AudioBot.ResourceFactories.Youtube
{
	public sealed class YoutubeResolver : IResourceResolver, IPlaylistResolver, IThumbnailResolver, ISearchResolver
	{
		...
		public R<PlayResource, LocalStr> GetResource(ResolveContext _, string uri)
		{
			...
			return GetResourceById(null, matchYtId.Groups[3].Value, null);
		}
		...
		public R<PlayResource, LocalStr> GetResourceById(ResolveContext _, string resourceId, string title)
		{
			...
			case LoaderPriority.Internal:
				var result = ResolveResourceInternal(resourceId, title);
				if (result.Ok)
					return result;
				goto case LoaderPriority.YoutubeDl;

			case LoaderPriority.YoutubeDl:
				return YoutubeDlWrapped(resourceId, title, ResolverFor);
		...
		private R<PlayResource, LocalStr> ResolveResourceInternal(string resourceId, string title)
		{
			if (!WebWrapper.DownloadString(out string resulthtml, new Uri($"https://www.youtube.com/get_video_info?video_id={resourceId}")))
			...
			title = title ?? $"<YT - no title : {resourceId}>";

			return new PlayResource(videoTypes[codec].Link, new AudioResource(resourceId, title, ResolverFor));
		}

		private static R<PlayResource, LocalStr> ParseLiveData(string resourceId, string title, string audioType, JsonPlayerResponse parsed)
		{
			...
			if (webListResponse.Ok)
			{
				...
				{
					title = title ?? parsed.videoDetails.title;
					return new PlayResource(streamSelect.TrackUrl, new AudioResource(resourceId, title, audioType));
				}
			}
			return new LocalStr(strings.error_media_no_stream_extracted);
		}
		...
		private static R<PlayResource, LocalStr> YoutubeDlWrapped(string resourceId, string title, string audioType)
		{
			...
			title = title ?? response.title ?? $"Youtube-{resourceId}";

			Log.Debug("youtube-dl succeeded!");
			return new PlayResource(url, new AudioResource(resourceId, title, audioType));
		}
