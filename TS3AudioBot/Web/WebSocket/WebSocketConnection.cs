...
namespace TS3AudioBot.Web.WebSocket {
	public class WebSocketConnection {
			...
			public void SendString(string message) {
				...
				if (running) {
					ComposeAndSend(1, Encoding.UTF8.GetBytes(message));
				}
			}
			...
			private string GetClientId() {
				return $"{((IPEndPoint) client.Client.LocalEndPoint).Port.ToString()}:{Uid}:{((IPEndPoint) client.Client.RemoteEndPoint).Port.ToString()}";
			}
